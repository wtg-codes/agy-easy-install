...
When installing the `agy-box` container sandbox on APT-based distributions (Debian/Ubuntu), the `agy-easy-install` script checks for `distrobox` and `podman`.

If they are missing, it will automatically prompt the user to install them via `sudo apt update && sudo apt install -y distrobox podman` before proceeding with the sandbox installation.
