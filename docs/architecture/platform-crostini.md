...
`agy-box` requires `distrobox` and a container runtime (`podman` or `docker`). Crostini environments are technically nested containers themselves (LXC). While running Podman inside Crostini is possible, performance and storage overhead should be considered.

The `agy-easy-install` script can attempt to install the necessary prerequisites if they are missing via APT (since Crostini runs Debian).
