...
The `agy-box` container environment can be run on Windows **via WSL2**.

### Prerequisites for `agy-box` on WSL2:
1. **Container Runtime:** Install Docker or Podman within your WSL2 Linux distribution.
2. **Distrobox:** Install Distrobox manually using its installation script:
   - `curl -s https://raw.githubusercontent.com/89luca89/distrobox/main/install | sh`

The `agy-easy-install` script detects WSL2 and will halt and prompt the user to install these prerequisites manually if they are not found.
