...
## Antigravity Developer Sandbox (`agy-box`)

The `agy-box` container environment is supported on macOS, but it requires additional setup due to macOS not having a native Linux kernel.

### Prerequisites for `agy-box` on macOS:
1. **Container Runtime:** Install a hypervisor-based container tool like Podman Desktop or Docker Desktop.
   - *Example:* `brew install --cask podman-desktop`
2. **Distrobox:** Install Distrobox via Homebrew.
   - *Example:* `brew install distrobox`

The `agy-easy-install` script will detect if you are on macOS and check for these prerequisites. If missing, it will display a guidance card outlining the required steps. It will not attempt to install them automatically.
