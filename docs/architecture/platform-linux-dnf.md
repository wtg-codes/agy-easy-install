...
When installing the `agy-box` container sandbox on DNF-based distributions (Fedora/RHEL), the `agy-easy-install` script checks for `distrobox` and `podman`.

If they are missing, it will automatically prompt the user to install them via `sudo dnf install -y distrobox podman` before proceeding with the sandbox installation.
