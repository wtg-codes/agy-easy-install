...
The `agy-box` isolated lab environment runs natively on Linux.

**Prerequisites:**
- **Container Runtime:** `podman` (preferred) or `docker`
- **Container Wrapper:** `distrobox`

The `agy-easy-install` script is capable of automatically installing these prerequisites via APT or DNF on standard Linux distributions. For Atomic variants, these are typically pre-installed.
