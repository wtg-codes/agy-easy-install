...
Atomic Linux variants (like Bluefin) are the ideal environments for `agy-box` because **Distrobox and Podman are pre-installed by default.**

The `agy-easy-install` script natively detects these tools and seamlessly proceeds with the `agy-box` sandbox initialization. The user may simply need to ensure their account has proper socket permissions if required (e.g., via `ujust dx-group`).
